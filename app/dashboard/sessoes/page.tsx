...
type Session = {
  ...
  mentor: { name: string | null; username: string | null; photo_url: string | null } | null;
};
...
function SessionRow({ session, onCancel }: { session: Session; onCancel: (id: string) => void }) {
  ...
  return (
    <div style={{
      ...
      <Avatar name={mentorName} photoUrl={session.mentor?.photo_url} />

      <div style={{ flex: 1, minWidth: 0 }}>
        <p style={{ fontSize: 14, fontWeight: 600, color: DARK, margin: "0 0 2px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{mentorName}</p>
      ...
      <section>
        ...
        {upcoming.length === 0 ? (
          <div style={{ padding: "28px 20px", background: BEIGE, borderRadius: 12, textAlign: "center" }}>
            ...
            <p style={{ fontSize: 12, color: FAINT, margin: 0 }}>Explore mentores e agende sua próxima sessão.</p>
          </div>
        ) : (
